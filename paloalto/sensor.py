"""
@Author:                Suresh Kalavala (https://github.com/skalavala/mysmarthome)
@Date:                  03/03/2018

GLOBALPROTECT FIX
@Customized by:         Step
@Customization date:    18/07/2022

PA-440 TEMP SENSOR FIX
@Customized by:         Step
@Customization date:    17/05/2023
...
PA_CONF_GP_USERS = "<show><global-protect-gateway><current-user/>" \
...
MONITORED_CONDITIONS = {
    'host_name': ['Hostname', 'x', 'mdi:fire'],
    'up_time': ['Uptime', 'x', 'mdi:clock'],
    'serial_no': ['Serial Number', 'x', 'mdi:counter'],
    'sw_version': ['Software Version', 'x', 'mdi:counter'],
    'gp_version': ['GlobalProtect Version', 'x', 'mdi:counter'],
    'logdb_version': ['LogDB Version', 'x', 'mdi:book-open'],
    'operation_mode': ['Operation Mode', 'x', 'mdi:book-open'],
    'cpu_temp': ['CPU die Temperature', 'x', 'mdi:oil-temperature'],
    'gp_user_count': ['GlobalProtect User Count', 'vpn users', 'mdi:counter'],
    'gp_users': ['GlobalProtect Users', 'vpn users', 'mdi:account-multiple'],
...
class PaloAltoApi(object):
    ...
    def parse_temperature(self):
        ...
        self._sensors["cpu_temp"] = round(float(nodes[0].text), 2)
